...
// Marker mit Koordinaten, Name, Temperatur, Datum, Link

    const markerOption = {
        title: "Wetterstationen in Tirol",                     
    opacity : 0.9,                       
}
for (let i = 0; i < awsdata.length; i++) {
    wetterstation = awsdata[i];
    L.marker([wetterstation.lat, wetterstation.lng], markerOption).addTo(markerGroup).bindPopup
    (wetterstation.name + "<br/> " + "Temperatur in C°: " + wetterstation.temperatur + "<br/>"
    + "Datum: " +  wetterstation.datum + "<br/>" + 
     "<a href=" + wetterstation.link + ">'Link zur Station'</a>");
}; 
